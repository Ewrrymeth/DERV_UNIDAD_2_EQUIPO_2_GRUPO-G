...
public class Fuerzas_imp : MonoBehaviour
{
    [SerializeField]
    Rigidbody rb;
    ...
    void Update()
    {
        ...
        {
            rb.AddForce(transform.right * -1 * velocidad, ForceMode.Impulse);
        }
        if (Input.GetKeyDown(KeyCode.S))
        {
            rb.AddForce(transform.forward * -1 * velocidad, ForceMode.Impulse);
        }
